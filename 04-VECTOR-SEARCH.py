...
results = vsc.get_index(index_name=vs_index_fullname, endpoint_name=vs_endpoint_name).similarity_search(
  query_text="Current league MLS, age 22, dependable striker, quick footwork and aggressive passing",
  columns=["player_id", "document"],
...
results_columns=["player_id", "document"]
...
df=spark.sql(f'''
             select 
                ...
                document_num_chars,
                document_num_words 
              from {catalog}.{schema}.player_documents_silver
                except 
              select  
                document, 
                player_name,
